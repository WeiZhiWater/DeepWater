    ...
    if eval(optData['name']) is hydroDL.data.dbCsv.DataframeCsv:
        ...
        if readY is True:
            y = df.getDataTs(
                ...
                doNorm=optData['doNorm'][1],    # Wei note: 'true', from doNorm=[True, True] in optDataCamels
                rmNan=optData['rmNan'][1])      # Wei note: 'false', from rmNaN=[True, False] in optDataCamels
        else:
            y = None
        ...
        if readX is True:
            x = df.getDataTs(
                varLst=optData['varT'],
                doNorm=optData['doNorm'][0],    # Wei note: 'true', from doNorm=[True, True] in optDataCamels
                rmNan=optData['rmNan'][0])      # Wei note: 'true', from rmNaN=[True, False] in optDataCamels
            c = df.getDataConst(
                varLst=optData['varC'],
                doNorm=optData['doNorm'][0],    # Wei note: 'true', from doNorm=[True, True] in optDataCamels
                rmNan=optData['rmNan'][0])      # Wei note: 'true', from rmNaN=[True, False] in optDataCamels
            ...
            x = None
            c = None
    elif eval(optData['name']) is hydroDL.data.camels.DataframeCamels:    # Wei note: this is the case
